...
export async function GET(request: Request) {
  try {
    ...
    const result = await adminApplication.inspectVnpayReturn(query);
    const transactionReference = query.get("vnp_TxnRef") ?? "";
    const orderNumber = /^MB-[0-9A-F]{20}$/.test(transactionReference)
      ? transactionReference
      : undefined;
    if (!result.valid) {
      return returnPage(
        "Không thể xác minh kết quả",
        "Thông tin trả về không hợp lệ. Đơn hàng chưa được đánh dấu đã thanh toán.",
        400,
      );
    }
    if (result.status === "SUCCESS") {
      return returnPage(
        ...
        "Chúng tôi đang xác nhận giao dịch qua kênh IPN bảo mật. Trạng thái đơn sẽ được cập nhật sau khi xác minh hoàn tất.",
        200,
        orderNumber,
      );
    }
    return returnPage(
      ...
      "Đơn hàng chưa được đánh dấu đã thanh toán. Bạn có thể quay lại cửa hàng và thử lại nếu cần.",
      200,
      orderNumber,
    );
  } catch {
