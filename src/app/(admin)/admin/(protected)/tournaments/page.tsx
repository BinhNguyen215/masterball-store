...
export default async function TournamentsPage({ searchParams }: { searchParams: Promise<AdminSearchParams> }) {
  ...
  return (
    <>
      ...
      <SearchFilterBar placeholder="Tên hoặc slug giải đấu" statusOptions={[{ value: "DRAFT", label: "Bản nháp" }, { value: "SCHEDULED", label: "Đã lên lịch" }, { value: "PUBLISHED", label: "Đã xuất bản" }, { value: "ARCHIVED", label: "Lưu trữ" }]} />
      ...
      <div className="mt-7 grid gap-5 xl:grid-cols-2">
        <MutationForm action={tournamentAction} submitLabel="Lưu bản nháp">
          ...
          <div className="grid gap-4 sm:grid-cols-2"><Field label="Slug"><input className={inputClassName} name="slug" required maxLength={180} pattern="[a-z0-9]+(?:-[a-z0-9]+)*" /></Field><Field label="Mã game"><input className={inputClassName} name="gameId" required maxLength={128} /></Field></div>
        ...
        <MutationForm action={tournamentAction} submitLabel="Lưu trạng thái">
          ...
          <Field label="Mã giải đấu"><input className={inputClassName} name="tournamentId" required maxLength={128} /></Field>
          <Field label="Trạng thái"><select className={inputClassName} name="status"><option value="DRAFT">Bản nháp</option><option value="SCHEDULED">Lên lịch</option><option value="PUBLISHED">Xuất bản ngay</option><option value="UNPUBLISHED">Gỡ xuất bản</option><option value="CANCELLED">Hủy</option><option value="ARCHIVED">Lưu trữ</option></select></Field>
          <Field label="Trạng thái hiện tại"><select className={inputClassName} name="currentStatus"><option value="DRAFT">Bản nháp</option><option value="SCHEDULED">Đã lên lịch</option><option value="PUBLISHED">Đã xuất bản</option><option value="UNPUBLISHED">Đã gỡ</option><option value="ARCHIVED">Lưu trữ</option></select></Field>
          <Field label="Thời điểm xuất bản (nếu lên lịch)"><input className={inputClassName} name="publishAt" type="datetime-local" /></Field>
