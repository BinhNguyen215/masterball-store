...
export default async function InventoryPage({ searchParams }: { searchParams: Promise<AdminSearchParams> }) {
  ...
  return (
    <>
      ...
      <SearchFilterBar placeholder="SKU hoặc tên sản phẩm" searchParams={query} statusOptions={[{ value: "LOW", label: "Sắp hết" }, { value: "OUT", label: "Hết hàng" }, { value: "AVAILABLE", label: "Còn hàng" }]} />
      ...
      {hasCapability(actor.role, "inventory.adjust") ? <div className="mt-7 max-w-2xl">
        <MutationForm action={inventoryAction} submitLabel="Ghi điều chỉnh">
          ...
          <Field label="Chênh lệch (+/−)"><input className={inputClassName} name="delta" type="number" min={-100000} max={100000} step={1} defaultValue={0} /></Field>
          <Field label="Ngưỡng cảnh báo sắp hết (để trống nếu không đổi)"><input className={inputClassName} name="reorderPoint" type="number" min={0} max={1000000} step={1} /></Field>
          <Field label="Lý do"><textarea className={inputClassName} name="reason" minLength={3} maxLength={240} rows={3} required /></Field>
          <p className="text-xs text-slate-500">Biểu mẫu không ghi SQL trực tiếp và không thể làm available âm. Đặt ngưỡng cảnh báo để bộ lọc “Sắp hết” hoạt động.</p>
        </MutationForm>
      </div> : null}
    </>
  );
